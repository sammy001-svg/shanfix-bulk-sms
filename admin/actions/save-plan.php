...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $units > 0) {
        try {
            if ($id) {
                DB::execute("UPDATE pricing_plans SET name = ?, units = ?, price = ?, currency = ?, is_popular = ? WHERE id = ?", [$name, $units, $price, $currency, $popular, $id]);
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Pricing plan updated successfully!'];
            } else {
                DB::execute("INSERT INTO pricing_plans (name, units, price, currency, is_active, is_popular, created_at) VALUES (?, ?, ?, ?, 1, ?, NOW())", 
        ...
        $_SESSION['flash'] = ['type' => 'danger', 'message' => 'All required fields must be filled correctly (Units must be > 0).'];
    }
