<?php
/**
 * WhatsApp Hub - Contact & Group Actions
 */
...
switch ($action) {
    ...
    case 'delete_group':
        // GET request for simplicity in deletion from sidebar
        if (!csrf_verify()) {
            flash_set('danger', 'Invalid security token.');
        } else {
            $id = (int)$_GET['id'];
    ...
    case 'import_contacts':
        // This will be handled via a POST from the frontend (processed rows)
...
redirect('/client/whatsapp-contacts.php' . ($groupId ? "?group=$groupId" : ""));
