...
if (isset($_POST['regenerate']) && csrf_verify()) {
    ...
    header('Location: /client/api.php');
    exit;
}
  ...
  <div class="card-body">
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
      ...
      <div style="background:var(--bg-dark);border-radius:var(--radius-md);padding:18px">
        ...
        <div style="font-size:13px;color:var(--text-secondary)">Go to <a href="/client/sender-ids.php">Sender IDs</a> and request one. Without an approved Sender ID you cannot send messages.</div>
      </div>
      <div style="background:var(--bg-dark);border-radius:var(--radius-md);padding:18px">
...
<pre class="code-block">{
  ...
  <span class="c-key">"currency"</span>:    <span class="c-str">"KES"</span>
    ...
    <section>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
          <div class="code-label">Single message</div>
          <div class="lang-panel" data-lang="php">
<pre class="code-block"><span class="c-kw">&lt;?php</span>
...
<span class="c-cm">// Single</span>
<span class="c-kw">const</span> r1 = <span class="c-kw">await</span> fetch(
  <span class="c-str">'<?= $base ?>/api/v1/status.php?message_id=4821'</span>,
  { headers: hdrs }
);

<span class="c-cm">// Batch</span>
<span class="c-kw">const</span> r2 = <span class="c-kw">await</span> fetch(
  <span class="c-str">'<?= $base ?>/api/v1/status.php?message_ids=4821,4822,4823'</span>,
  { headers: hdrs }
);
console.log(<span class="c-kw">await</span> r2.json());</pre>
          </div>
        </div>
        <div>
...
.c-kw  { color: #7dd3fc; }  /* keyword / bool */
.c-str { color: #86efac; }  /* string */
.c-num { color: #fbbf24; }  /* number */
.c-key { color: #93c5fd; }  /* object key */
.c-cm  { color: #475569; }  /* comment */
</style>
